...
class BWCircularSlider: UIControl {
    ...
    override init(frame: CGRect) {
        ...
        let textFieldRect = CGRectMake(
            (frame.size.width - fontSize.height) / 2.0,
            (frame.size.height - fontSize.width) / 2.0,
            fontSize.width,
            fontSize.height);
    ...
    override func drawRect(rect: CGRect) {
        ...
        UIGraphicsBeginImageContext(CGSizeMake(self.bounds.width, self.bounds.height))
        ...
        // Split colors in the component (rgba)
    ...
    func moveHandle(lastPoint:CGPoint) {
        ...
        // Calculate the direction frmo the center point and an arbitrary position
    ...
    func pointFromAngle(angleInt:Int) -> CGPoint {
        // Circle center
        let centerPoint = CGPointMake(self.frame.width / 2.0 - Config.TB_LINE_WIDTH / 2.0, self.frame.height / 2.0 - Config.TB_LINE_WIDTH / 2.0)
